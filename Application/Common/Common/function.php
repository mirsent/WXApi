 ...
 * 计算两点行驶距离
 * @param   $origins      起点经纬度
 * @param   $destinations 终点经纬度
 * @param   $type         方式：driving 驾车，riding 骑行，walking 步行
 * @return array          distance 距离，duration 耗时
 */
function get_routematrix($origins, $destinations, $type='riding'){
    $ak = 'wlglXRWsdibjGr74gQj2XZ4tKdLvnLSQ';
    $url = "http://api.map.baidu.com/routematrix/v2/".$type."?output=json&origins=".$origins."&destinations=".$destinations."&ak=".$ak;
...
function upload_multiple_img($path){
    $path=trim($path,'.');
    $path=trim($path,'/');
    $config = array(
        'rootPath' => './UploadImages/',
 ...
 */
function exportExcel($fileName='table',$expCellName,$expTableData){
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', $fileName.'  时间:'.date('Y-m-d H:i:s'));
...
function numberVal($str){
    return preg_replace("/(?:\.0*|(\.\d+?)0+)$/",'$1', $str);
}
